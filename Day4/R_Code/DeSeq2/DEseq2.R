...
newdata <- read.delim("res_output.csv", sep = ',')
...
# *************************
#  Plot Volcano Graphs
p1 <- ggplot(newdata, aes(log2FoldChange, -log(padj,10))) + # -log10 conversion  
  geom_point(size = 2/5) +
...
q <- read.delim("/Users/jeffersonchen/programming/YuceBio/star-fusion.fusion_predictions.abridged.coding_effect.filtered.anno.tsv")
